    ...
    if (g_usbCommsInitialized) {
        ...
        if (R_SUCCEEDED(rc)) {
            if (hosversionAtLeast(5,0,0)) {
                ...
                // Send product
                if (R_SUCCEEDED(rc)) rc = usbDsAddUsbStringDescriptor(&iProduct, "Transferencia Switch");
                // Send serial number
                if (R_SUCCEEDED(rc)) rc = usbDsAddUsbStringDescriptor(&iSerialNumber, "TS-MTP-0001");
...
static Result _usbCommsTransfer(usbCommsEndpoint *ep, UsbDirection dir, const void* buffer, size_t size, u64 timeout, size_t *transferredSize)
{
    ...
    // Use the caller timeout here as well. Otherwise an unplugged host blocks forever
    // before the cancellable endpoint wait is reached.
